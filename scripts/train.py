...
def save_loss_plot(train_losses, val_losses, output_path: Path):
    ...
    plt.plot(epochs, train_losses, label="Train Loss (MAE)", color="#1f77b4", linewidth=2.0, alpha=0.85)
    plt.plot(epochs, val_losses, label="Validation Loss (MAE)", color="#ff7f0e", linewidth=2.0, alpha=0.85)
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
...
def save_accuracy_plot(train_accuracies, val_accuracies, output_path: Path):
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(epochs, train_accuracies, label="Train Accuracy", color="#1f77b4", linewidth=2.0, alpha=0.85)
    plt.plot(epochs, val_accuracies, label="Validation Accuracy", color="#ff7f0e", linewidth=2.0, alpha=0.85)
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
...
def main(
    ...
    if IS_COLAB:
        args = get_args_colab(
            ...
            weight_decay=weight_decay or 4e-4,
            batch_size=batch_size or 128,
            hidden_dim=hidden_dim or 96,
            num_layers=num_layers or 3,
            heads=heads or 4,
            dropout=dropout or 0.22,
            patience=patience or 20,
            min_epochs_before_stop=min_epochs_before_stop or 115,
        ...
        parser.add_argument("--epochs", type=int, default=155)
        ...
        parser.add_argument("--min-epochs-before-stop", type=int, default=115)
