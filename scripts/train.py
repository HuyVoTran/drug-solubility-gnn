...
def _ema(values: list[float], alpha: float = 0.12) -> list[float]:
    """Exponential moving average for smoother plot lines (α=0.12 balances smoothness and detail)."""
    smoothed = []
    s = values[0]
    for v in values:
        s = alpha * v + (1 - alpha) * s
...
def main(
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=0.5, patience=4, min_lr=1e-5)
    criterion = nn.L1Loss()
